---
title: 纯CSS实现鼠标文本触发 tip 气泡效果
description: 纯CSS实现鼠标文本触发 tip 气泡效果
tags: 
  - CSS,attr(),transform
head:
  - - meta
    - name: keywords
      content: CSS,attr(),transform
---

# 纯CSS实现鼠标文本触发 tip 气泡效果

使用`attr()`属性读取 Tip 气泡内容，`before`、`after`伪类实现气泡样式，`transform`属性实现气泡出现动画和位置。
